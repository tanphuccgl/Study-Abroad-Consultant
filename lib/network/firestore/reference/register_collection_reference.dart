...
class RegisterCollectionReference extends BaseCollectionReference<Register> {
  RegisterCollectionReference()
      : super(
          FirebaseFirestore.instance
              .collection("register")
              .withConverter<Register>(
                fromFirestore: ((snapshot, _) =>
                    Register.fromDocument(snapshot)),
                toFirestore: (model, _) => model.toMap(),
              ),
        );

  Future<XResult<bool>> postWGroup(Register group) async {
    try {
  ...
  Future<XResult<List<Register>>> getListGroup() async {
    try {
  ...
  Future<XResult<Register>> getInfoGroup(String id) async {
    try {
